package com.example.circularimage.circularimage;
...
public class CircularImageView extends AppCompatImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textPaint.setTextSize(context.getResources().getDimensionPixelSize(R.dimen.body_default_font_size));
    ...
    public void onDraw(Canvas canvas) {
        ...
        // When there is no profile pic then showing title text in center of image.
        if (isNeedToShow && text != null) {
            int xPos = (canvas.getWidth() / 2);
            int yPos = (int) ((canvas.getHeight() / 2) - ((textPaint.descent() + textPaint.ascent()) / 2));

            canvas.drawText(text.toUpperCase(), xPos, yPos, textPaint);
        } else {
    ...
    /**
     * Method to set text size of text paint.
     * @param dimensionPixelSize Size of the text.
     */
    public void setTextSize(int dimensionPixelSize) {
        if (textPaint != null) {
    ...
    /**
     * Method to hide text from image.
     */
    ...
    /**
     * Method to show text on ImageView.
     * @param text Title text from which first letter need to be shown in image view.
     * @param color Color of the background shape.
     */
    public void showText(String text, int color) {
        ...
        if (text != null && text.trim().length() > 0) {
            ...
            for (String s : text.trim().split("\\s+")) {
                initials.append(s.trim().charAt(0));
            }
            this.text = initials.length() > 2 ? initials.substring(0, 2) :
                    (initials.length() == 1 && text.trim().length() >= 2 ? text.trim().substring(0, 2) : initials.toString());
            Rect rectText = new Rect();
            textPaint.getTextBounds(this.text, 0, this.text.length(), rectText);
        } else {
            this.text = null;
        }
        Drawable drawable = ContextCompat.getDrawable(mContext, R.drawable.imageshape);
